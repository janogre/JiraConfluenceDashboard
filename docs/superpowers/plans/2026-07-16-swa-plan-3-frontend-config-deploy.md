...
3. **CORS (sikkerhetskritisk, fra Plan 2-review):** på AI Function App-en, sett CORS-origin-allowlist til **kun** SWA-domenet (`https://<domene>`). Dette er den reelle sikkerhetsperimeteren siden function-key ligger i klient-bundelen. **Verifiser preflight:** siden klienten sender en custom `x-functions-key`-header, gjør nettleseren en CORS-**preflight** (OPTIONS). Azure Functions' innebygde CORS reflekterer normalt forespurte headere automatisk, men bekreft med et **ekte nettleser-AI-kall** fra SWA-domenet (ikke bare curl) at preflight lykkes og `x-functions-key` godtas.
